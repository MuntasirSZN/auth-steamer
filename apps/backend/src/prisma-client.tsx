...
import {
	getEnvVariable,
	getNodeEnvironment,
} from "@stackframe/stack-shared/dist/utils/env";
import {
	filterUndefined,
	typedFromEntries,
	typedKeys,
} from "@stackframe/stack-shared/dist/utils/objects";
...
const useAccelerate =
	getEnvVariable("STACK_ACCELERATE_ENABLED", "false") === "true";
...
export async function rawQuery<Q extends RawQuery<any>>(
	query: Q,
): Promise<Awaited<ReturnType<Q["postProcess"]>>> {
	const result = await rawQueryArray([query]);
	return result[0];
}

export async function rawQueryAll<
	Q extends Record<string, undefined | RawQuery<any>>,
>(
	queries: Q,
): Promise<{
	[K in keyof Q]: Awaited<ReturnType<NonNullable<Q[K]>["postProcess"]>>;
}> {
	const keys = typedKeys(filterUndefined(queries));
	const result = await rawQueryArray(
		keys.map((key) => queries[key as any] as any),
	);
	return typedFromEntries(
		keys.map((key, index) => [key, result[index]]),
	) as any;
}

async function rawQueryArray<Q extends RawQuery<any>[]>(
	queries: Q,
): Promise<[] & { [K in keyof Q]: Awaited<ReturnType<Q[K]["postProcess"]>> }> {
	return await traceSpan(
		{
			...
			attributes: {
				...
				...Object.fromEntries(
					queries.map((q, index) => [`stack.raw-queries.${index}`, q.sql.text]),
				),
			},
		},
		async () => {
    ...
    `;
			...
			const rawResult = (await prismaClient.$queryRaw(query)) as {
				type: string;
				json: any;
			}[];
			const unprocessed = new Array(queries.length)
				.fill(null)
				.map(() => [] as any[]);
			...
			const postProcessed = queries.map((q, index) =>
				q.postProcess(unprocessed[index]),
			);
